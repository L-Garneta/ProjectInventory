...
async function renderTable() {
  ...
  tbody.innerHTML = `<tr><td colspan="${admin ? 8 : 7}">Loading...</td></tr>`;

  const items = await getItems();

  if (!items.length) {
    tbody.innerHTML = `<tr><td colspan="${admin ? 8 : 7}">Belum ada data</td></tr>`;
    return;
  }
    ...
    <tr>
      ...
      ${
        admin
          ? `
      <td>
        <button class="btn btn-warning btn-sm" data-edit="${item.id}">Edit</button>
        <button class="btn btn-danger btn-sm" data-delete="${item.id}">Hapus</button>
      </td>
      `
          : ""
      }
